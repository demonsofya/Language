...
#define SYNTAX_ERROR { SyntaxErrorFunction( __FILE__, __FUNCTION__, __LINE__); assert(NULL); }
...
char *curr_input_file_name = NULL;

#define DEFAULT_INPUT_FILE_FOR_TREE "input.txt"
#define SYNTAX_RULES_FILE_NAME "../правила ЕГЭ информатика 2026.txt"
...
Tree_t *GetTokensGraph(char *input_file_name) {

    size_t buffer_size = 0; 
    char *buffer = NULL;
    curr_input_file_name = input_file_name;

    if (input_file_name != NULL)
        buffer = GetBuffer(input_file_name, &buffer_size);
    else
        buffer = GetBuffer(DEFAULT_INPUT_FILE_FOR_TREE, &buffer_size);
...
void SyntaxErrorFunction(const char *file_name, const char *function_name, int line_number) {
    ...
    if (curr_input_file_name != NULL)   
        input_file_ptr = fopen(curr_input_file_name, "w+");
    else    
        input_file_ptr = fopen(DEFAULT_INPUT_FILE_FOR_TREE, "w+");

    if (file_name != NULL && function_name != NULL)
        fprintf(input_file_ptr, "ВЫ ДОПУСТИЛИ ОШИБКУ СИНТАКСИЧЕСКУЮ ОШИБКУ.\nВ ФАЙЛЕ %s В ФУНКЦИИ %s НА ЛИНИИ %d.\nПОЖАЛУЙСТА, СВЕРЬТЕСЬ С ПРАВИЛАМИ ЯЗЫКА\n",
                                 file_name, function_name, line_number);
    fprintf(input_file_ptr, "\n%s", language_rules);
}
